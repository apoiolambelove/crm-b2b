  ...
  try {
    ...
    if (event.httpMethod === 'PATCH') {
      ...
      const { data: pedidoAtual, error: getErr } = await supabase
        .from('pedidos')
        .select('id, numero_pedido, status, pagamento_comissao_id, repasse_consignacao_id')
        .eq('id', body.id)
        .maybeSingle();
      if (getErr) throw getErr;
      ...
      // Mesma trava, agora para o fechamento semanal de Consignação: um pedido
      // que já entrou em um repasse (fechamento) não pode ter o status alterado,
      // senão o estoque e o valor já fechado ficam inconsistentes.
      if (pedidoAtual.repasse_consignacao_id && body.status !== pedidoAtual.status) {
        return fail(
          'Este pedido já entrou em um fechamento semanal de Consignação e não pode ter o status alterado.',
          409
        );
      }
