...
│   ├── consignacao.js             # estoque em consignação + repasse semanal
...
5. Se o seu banco já existia antes do módulo de Consignação, rode também o arquivo
   `supabase/migration_consignacao.sql` (uma única vez, é seguro rodar de novo se precisar).
   Ele cria as tabelas `consignacao_movimentos` e `repasses_consignacao`, a view
   `vw_estoque_consignacao` e o gatilho que baixa/estorna o estoque automaticamente
   quando um pedido muda de/para "Venda Concluída".

### Módulo de Consignação

Nova aba **Consignação** (menu lateral, visível para Master/Admin e vendedoras):

- **Master**: registra a quantidade entregue fisicamente a cada vendedora (fica em
  "Estoque em Consignação"), acompanha o estoque de todas descendo sozinho conforme
  os pedidos delas são marcados como "Venda Concluída" (função `pedidos.js`, aba
  Pedidos), e fecha a semana (recomendado: toda quarta-feira) somando os pedidos
  concluídos ainda não fechados de cada vendedora. O fechamento calcula o valor
  vendido, a comissão que ela retém (15%, ou o percentual do pedido) e o valor que
  ela deve repassar para a empresa (o restante). Depois de a vendedora entregar o
  dinheiro, o Master clica em "Confirmar recebimento".
- **Vendedora**: vê o próprio estoque restante e o valor que tem a repassar na
  semana atual, além do histórico de repasses já fechados (pendente/recebido).

O estoque nunca é alterado diretamente pelo frontend: um gatilho no banco
(`trg_pedidos_consignacao`) lança a baixa (ou o estorno, se o status voltar atrás)
sozinho sempre que o status do pedido muda — assim não tem como o estoque
dessincronizar dos pedidos reais. Um pedido que já entrou em um fechamento semanal
fica travado (não é mais possível mudar o status dele) até que o fechamento seja
cancelado pelo Master.
